...
All numbers below are generated from the code by `tools/scorecard.py`; CI fails if this section and the code disagree.

<!-- scorecard:begin (generated by tools/scorecard.py, do not edit by hand) -->
...
**Mass coordinates.** Unconfined fermions (leptons, top) are at the propagator pole; confined heavy quarks (c, b) at the self-scale `m(m)`; light quarks (u, d, s) at the PDG `MS-bar(2 GeV)` coordinate. The scheme-free content of the light sector is the RG-invariant ratios, which carry no coordinate at all: `m_u/m_d = 38/83` (-0.9σ vs PDG 0.473(17)), `m_s/m_ud = 27.130` (-2.1σ vs 27.30(8), a registered watch), `Q_ellipse = 22.229` (+0.2σ dispersive, -2.0σ lattice; the two references disagree). See `masses.py` for the full treatment.

### Mixing, couplings, gravity, baryogenesis, dark sector

| Sector | Result | Reference | Status |
|:---|:---|:---|:---|
| Electroweak scale `v_EW` | 246.21965 GeV | 246.21965 GeV (from G_F) | 0.02 ppm |
| Fine structure `1/α(0)` | 137.035999050 | Berkeley Cs 137.035999046(27) | pull +0.13σ (Cs commitment, Rb kill condition registered) |
| Higgs mass `m_H` | 124.06 GeV | 125.20 GeV | -0.91% (2-loop band) |
| CKM (15 observables) | χ²/n = 0.69 | PDG 2024 | max pull 1.4σ |
| PMNS (3 observables) | χ²/n = 0.00 | NuFit 6.0 | predicts δ_CP = 76.9° |
| Strong coupling `α_s(M_Z)` | 0.1184 | PDG 0.1180(9) | +0.33% (+0.43σ; π/32 exact at μ* = 253.5 GeV) |
| Weinberg angle `sin²θ_W` | 0.231234 | PDG 0.23129(4) | pull -1.41σ |
| W mass `M_W` | 80.365 GeV | 80.3692(133) world avg | pull -0.33σ |
...
**Registered watches.** The framework bets ahead of the data and registers the kill conditions in [`watches/REGISTRY.md`](watches/REGISTRY.md): the 4/13 absence test on `α_s` (anomalous gauge dressing of ±(4/13)/(2π) would falsify the no-self-dilution symmetry), `m_s/m_ud` at −2.1σ (the sharpest mass-sector pull), the cesium commitment on `α(0)` (the two recoil experiments disagree by 5.5σ; landing on Rb kills the bridge term), the `sin²θ_W` emission weight (FCC-ee discriminable), and the `m_b` down-type vent.

**The precision hierarchy is structural.** Leptons sit closest to the `Z₃` source and emerge cleanly (`m_μ`, `m_τ` predicted from `m_e` to 0.0012%). Quarks pass through additional layers (triality, generation scaling, confinement), and each layer adds residuals of order `α_s/π` (≤ 0.3%). The Newton-constant ledger closes internally to 8 × 10⁻⁸ under the face-split law; the phase readings (UV/broken) bracket it at 0.0–1.3%. The accuracy gradient is not noise. It is what the emergence depth predicts.
...
Two independent routes converge on the Einstein equations with the same coefficient: a one-loop heat-kernel (Sakharov) calculation of the induced action, and a Jacobson–Clausius construction from horizon thermodynamics. Newton's `G` closes to 8 × 10⁻⁸ internally once the face-split no-self-dilution law is applied (the UV and broken-phase readings bracket it at 0.0–1.3%).
...
See [`CONTRIBUTING.md`](CONTRIBUTING.md). Short version: verification, critique, tooling, and pre-registered candidate derivations are welcome; corrections with anything adjustable in them are not, because there is nothing here to tune. The lifecycle is `candidates/` → (derivation meeting the promotion bar) → canon, with kills preserved in `killed/` and every canonical change recorded in [`CHANGELOG.md`](CHANGELOG.md).

## Citing

Companion papers, in dependency order (the papers are dated snapshots; this repository is the living canon, and divergences are recorded in [`CHANGELOG.md`](CHANGELOG.md) until the next paper version):
