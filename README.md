...
34 numerical predictions. Run it yourself:

```bash
python -m E8              # full layer-by-layer derivation
python -m E8 --summary    # scorecard + emergence tree only
```

## The scorecard

All numbers below come from running the code as-is. `M_Pl = 1.22089 × 10¹⁹ GeV` is used to express results in GeV.

### Masses (9/9, max error 0.3%)

| Particle | Predicted | PDG | Error | Method |
|---:|---:|---:|---:|:---|
| e | 0.5110 MeV | 0.5110 MeV | −0.002% | Brannen Z₃ |
| μ | 105.657 MeV | 105.658 MeV | −0.001% | Brannen Z₃ |
| τ | 1776.95 MeV | 1776.93 MeV | +0.001% | Brannen Z₃ |
| u | 2.158 MeV | 2.16 MeV | −0.1% | (38/9) m_e |
| d | 4.712 MeV | 4.70 MeV | +0.3% | (83/9) m_e |
| s | 93.2 MeV | 93.5 MeV | −0.3% | Rivero + √(32/27) bridge |
| c | 1273.8 MeV | 1273 MeV | +0.06% | (217/18) m_μ |
| b | 4194.0 MeV | 4183 MeV | +0.3% | Q = 2/3 + h₁₁/K³ |
| t | 172.51 GeV | 172.57 GeV | −0.03% | (1165/12) m_τ |

### Mixing, couplings, gravity, baryogenesis

| Sector | Result | Reference | Status |
|:---|:---|:---|:---|
| Electroweak scale `v_EW` | 246.21 GeV | 246.22 GeV | −0.00% |
| Higgs mass `m_H` | F₄ fusion + bridge | 125.10 GeV | within 0.1% |
| CKM (15 observables) | χ²/n = 0.69 | PDG 2025 | max pull 1.4σ |
| PMNS (3 observables) | χ²/n = 0.57 | NuFit 6.0 | predicts δ_CP = 77° |
| Strong coupling `α_s(M_Z)` | 0.1177 | PDG | −0.3% |
| Newton's constant `G_ind/G_N` | 0.9942 (UV) – 1.0075 (broken) | 1 | 0.6–0.7% |
...
The lepton mass formula is fixed entirely by the algebra of the loop and the octonionic `G₂/SU(3)` Clebsch–Gordan data: `B/A = √2`, `θ = 2/9`, scale `½ M_Pl × exp(−9π²/2)`. The Koide identity `Q = 2/3` is the algebraic shadow of this closure.
...
E₈(1) → G₂(1) × F₄(1)
  │
  ├── G₂ sector → leptons        (Brannen Z₃: B/A = √2, θ = 2/9, Q = 2/3)
  ...
  ├── SU(3)₃ / Z_C → PMNS        (tribimaximal + corrections)
  ├── F₄ singlets → ν masses     (m₁ = 0 structurally)
  ├── Embedding index → α_s
  ├── G₂ instanton → η_B
...
Each layer is a module exposing one `derive(deps) → dict` function. The order is explicit in `E8/__main__.py`.

| Module | Layer |
|:---|:---|
| `constants.py` | Unit-setting scale (M_Pl) + PDG/NuFit reference data |
| `formatting.py` | Terminal output helpers |
| `algebra.py` | Layer 0: `E₈ → G₂ × F₄` Casimirs, central charges, embedding indices |
| `scale.py` | Layer 1: `v_EW` via instanton suppression |
...
| `alpha_s.py` | Layer 7: Strong coupling via embedding index chain |
| `higgs.py` | Layer 8: Higgs mass from F₄(1) fusion + bridge threshold |
| `gravity.py` | Layer 10: Sakharov induced gravity, (7,26) heat kernel |
| `baryogenesis.py` | Layer 11: `η_B` from G₂ instanton + Fano orientation |
| `proofs.py` | Layer 12: Proof certificates |
